...
class OrderRepository extends BaseRepository
{
    ...
    public function createOrder($item, $request)
    {
        if ($this->isActivePaymentMethod($request->payment_method, $item['total']['total'])) {
            ...
            $order->products;
            $order->order_status;
            // Group products by created_by_id and concatenate product names
$data = [];

foreach ($order->products as $product) {
    $vendorId = $product['created_by_id'];
    $productName = $product['name'];

    // If the vendorId doesn't exist in $data, initialize it
    ...
    // Add product name to the vendor's list of products
    $data[$vendorId]['products_name'][] = $productName;
}

// Retrieve phone number and country code for each vendor and send WhatsApp notification
foreach ($data as $vendor) {

    $vendorId = $vendor['created_by_id'];
    $productsName = implode(', ', $vendor['products_name']);

    // Retrieve vendor's phone number and country code from the user table
    ...
    function sendWhatsAppNotification($countryCode, $phoneNumber, $productsName, $vendorName,$order_number) {
        $response = Http::withHeaders([
            ...
            // 'callbackData' => 'some text fsdg here',
            'type' => 'Template',
            'template' => [
